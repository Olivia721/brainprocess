function OutputFiles = my_connectivity(FilesA, FilesB, OPTIONS)
...
if isempty(OPTIONS.isSymmetric)
    OPTIONS.isSymmetric = any(strcmpi(OPTIONS.Method, {'corr','cohere','plv','plvt','aec','wpli'})) && (isempty(FilesB) || (isequal(FilesA, FilesB) && isequal(OPTIONS.TargetA, OPTIONS.TargetB)));
end
...
%             end
        ...
        % ==== WPLI ====
        case 'wpli'
            bst_progress('text', sprintf('Calculating: wPLI [%dx%d]...', size(sInputA.Data,1), size(sInputB.Data,1)));
            Comment = 'WPLI: ';
            % Get frequency bands
            nFreqBands = size(OPTIONS.Freqs, 1);
            BandBounds = process_tf_bands('GetBounds', OPTIONS.Freqs);
            
            % ===== IMPLEMENTATION F.VERBOSIO =====
            % Intitialize returned matrix
            R = zeros(size(sInputA.Data,1), size(sInputB.Data,1), nFreqBands);
            S = zeros(size(sInputA.Data,1), size(sInputB.Data,1), nFreqBands);
            % Loop on each frequency band
            for iBand = 1:nFreqBands
                % Band-pass filter in one frequency band + Apply Hilbert transform
                if isConnNN
                    DataAband = process_bandpass('Compute', sInputA.Data, sfreq, BandBounds(iBand,1), BandBounds(iBand,2), 'bst-hfilter-2019', OPTIONS.isMirror, 0, (BandBounds(iBand,1)+BandBounds(iBand,2))/2);
                    HA = hilbert_fcn(DataAband')';
                    HB = HA;
                else
                    DataAband = process_bandpass('Compute', sInputA.Data, sfreq, BandBounds(iBand,1), BandBounds(iBand,2), 'bst-hfilter-2019', OPTIONS.isMirror, 0, (BandBounds(iBand,1)+BandBounds(iBand,2))/2);
                    DataBband = process_bandpass('Compute', sInputB.Data, sfreq, BandBounds(iBand,1), BandBounds(iBand,2), 'bst-hfilter-2019', OPTIONS.isMirror, 0, (BandBounds(iBand,1)+BandBounds(iBand,2))/2);
                    HA = hilbert_fcn(DataAband')';
                    HB = hilbert_fcn(DataBband')';
                end
                phaseA = HA ./ abs(HA);
                ...
                for iCh = 1:nC
                    ...
                    for jCh = 1:nC
                        Xj = phaseB(jCh,:);
                        crossSpectr = Xi .* conj(Xj);
                        X = imag(crossSpectr);
                        wPLI(iCh,jCh) = abs(sum(X)) / sum(abs(X));
                        
                        % surrogate
                        offset = randi(nS,1);
                        crossSpectr = Xi .* circshift(Xj,offset);
                        X = imag(crossSpectr);
                        surr(iCh,jCh) = abs(sum(X)) / sum(abs(X));
                    end
                end
        ...
        case 'plv'
            ...
            for iBand = 1:nFreqBands
                ...
                for iCh = 1:nC
                    ...
                    for jCh = iCh+1:nC                  
                        Xj = phaseB(jCh,:);     
                        cPLV(iCh,jCh) = Xi * Xj' / size(phaseA,2);
                        % surrogate
                        offset = randi(nS,1);
                        surr(iCh,jCh) = Xi * circshift(Xj,offset)' / size(phaseA,2);
                    end
                end
               
                % Divide by number of time samples
                R(:,:,iBand) = cPLV + cPLV.'; %(cA*cB' + sA*sB' + 1i * (sA*cB' - cA*sB')) ./ size(cA,2); 
                S(:,:,iBand) = surr + surr.';
    ...
    if exist('S','var')
        S = squeeze(reshape(S, [], nTime, size(S,3)));
    end
    % Comment
    ...
    switch (OPTIONS.OutputMode)
        case 'input'
            nAvg = 1;
            OutputFiles{end+1} = SaveFile(R, sInputB.iStudy, FilesB{iFile}, sInputA, sInputB, [Comment,int2str(nC),'_obs'], nAvg, OPTIONS, FreqBands);
            if exist('S','var')
                OutputFiles{end+1} = SaveFile(S, sInputB.iStudy, FilesB{iFile}, sInputA, sInputB, [Comment,int2str(nC),'_sur'], nAvg, OPTIONS, FreqBands);
            end
        case 'concat'
            nAvg = 1;
